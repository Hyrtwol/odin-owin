...
set_dot :: proc(pos: owin.int2, col: owin.byte4) {
...
WM_CREATE :: proc(hwnd: owin.HWND, lparam: owin.LPARAM) -> owin.LRESULT {
...

	{
		...
		bitmap_handle = owin.create_dib_section(hdc, cast(^owin.BITMAPINFO)&bmi_header, owin.dib_usage.DIB_RGB_COLORS, &pvBits, nil, 0)
	}
...
WM_ERASEBKGND :: proc(hwnd: owin.HWND, wparam: owin.WPARAM) -> owin.LRESULT {
	return 1
}

WM_SETFOCUS :: proc(hwnd: owin.HWND, wparam: owin.WPARAM) -> owin.LRESULT {
	// wParam : A handle to the window that has lost the keyboard focus. This parameter can be NULL.
	fmt.println(#procedure, hwnd, owin.HWND(wparam))
	return 0
}

WM_KILLFOCUS :: proc(hwnd: owin.HWND, wparam: owin.WPARAM) -> owin.LRESULT {
	fmt.println(#procedure, hwnd, wparam)
	return 0
}

WM_CHAR :: proc(hwnd: owin.HWND, wparam: owin.WPARAM, lparam: owin.LPARAM) -> owin.LRESULT {
	switch wparam {
...
WM_SIZING :: proc(hwnd: owin.HWND, wparam: owin.WPARAM, lparam: owin.LPARAM) -> owin.LRESULT {
...
WM_PAINT :: proc(hwnd: owin.HWND) -> owin.LRESULT {
	ps: owin.PAINTSTRUCT
	...
	brush := owin.HBRUSH(win32.GetStockObject(win32.DC_BRUSH))
	...
	owin.select_object(hdc_source, bitmap_handle)
	win32.AlphaBlend(ps.hdc, 0, 0, client_size.x, client_size.y, hdc_source, 0, 0, bitmap_size.x, bitmap_size.y, ftn)

	return 0
}

WM_TIMER :: proc(hwnd: owin.HWND, wparam: owin.WPARAM, lparam: owin.LPARAM) -> owin.LRESULT {
	switch owin.UINT_PTR(wparam)
	{
	case owin.IDT_TIMER1: set_dot_invalidate(hwnd, random_scrpos(), COLOR_CYAN)
	case owin.IDT_TIMER2: set_dot_invalidate(hwnd, random_scrpos(), COLOR_YELLOW)
	}
	return 0
}

set_dot_invalidate :: proc(hwnd: owin.HWND, pos: owin.int2, col: owin.byte4) {
	set_dot(pos, col)
	owin.invalidate_window(hwnd)
}

decode_set_dot :: proc(hwnd: owin.HWND, lparam: owin.LPARAM, col: owin.byte4) {
	set_dot_invalidate(hwnd, decode_scrpos(lparam), col)
}

handle_input :: proc(hwnd: owin.HWND, wparam: owin.WPARAM, lparam: owin.LPARAM) -> owin.LRESULT {
	// odinfmt: disable
	mouse_key_state := owin.decode_wparam_as_mouse_key_state(wparam)
	switch mouse_key_state {
	case {.MK_LBUTTON}: decode_set_dot(hwnd, lparam, COLOR_RED)
	case {.MK_RBUTTON}: decode_set_dot(hwnd, lparam, COLOR_BLUE)
	case {.MK_LBUTTON, .MK_RBUTTON}: decode_set_dot(hwnd, lparam, COLOR_GREEN)
...
wndproc :: proc "system" (hwnd: owin.HWND, msg: owin.WM_MSG, wparam: owin.WPARAM, lparam: owin.LPARAM) -> owin.LRESULT {
